...
from app.utils.components import show_info, show_error
...
def render_overview_tab():
    ...
    with col1:
        st.metric(
            "Total Vendors",
            stats.get("total_vendors", 0),
            delta="Active Systems"
        )

    with col2:
        st.metric(
            "Total Systems",
            stats.get("total_systems", 0),
            delta="Validated"
        )
    ...
    with col4:
        st.metric(
            ...
            stats.get("pending_confirmations", 0),
    ...
    with col1:
        ...
        if trial_status:
            fig = px.pie(
                values=list(trial_status.values()),
                names=list(trial_status.keys()),
...
def render_analysis_tab():
    ...
    display_df = df[[
        "instance_code", "platform_name", "category_code",
