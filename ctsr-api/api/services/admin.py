...
from api.db.models import (
    Trial, SystemInstance, TrialSystemLink, Confirmation,
    ValidationStatus
)
from api.models.admin import (
    DashboardStats, TrialSummary, SystemSummary,
    ConfirmationSummary, ValidationAlertSummary, RecentActivity
)
...
class AdminService:
    ...
    async def _get_trial_stats(self) -> TrialSummary:
        ...
        active_result = await self.db.execute(
            ...
            .where(
                and_(
                    ...
                    TrialSystemLink.assignment_status == "ACTIVE"
                )
            )
        )
        ...
        return TrialSummary(
            ...
            trials_with_alerts=trials_with_alerts
        )
    ...
    async def _get_system_stats(self) -> SystemSummary:
        ...
        # Total systems
        total_result = await self.db.execute(
            select(func.count(SystemInstance.instance_id))
        )
    ...
    async def _get_confirmation_stats(self) -> ConfirmationSummary:
        ...
        # Total confirmations
        total_result = await self.db.execute(
            select(func.count(Confirmation.confirmation_id))
        )
        ...
        pending_result = await self.db.execute(
            select(func.count(Confirmation.confirmation_id))
            .where(Confirmation.confirmation_status == "PENDING")
        )
        ...
        completed_result = await self.db.execute(
            ...
            .where(
                and_(
                    ...
                    Confirmation.confirmed_date >= first_of_month
                )
            )
        )
